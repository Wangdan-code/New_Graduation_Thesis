...
其中，GRU、LSTM均基于Pytorch1.10实现，其参数设置为：\ding{172} 子树划分：采用AST-Block的划分方法。\ding{173} 子树的表征均采用基于树的卷积神经网络，整树的表征模型的隐藏层维度设置为128，模型使用二元交叉熵作为损失函数，使用Adam优化器来训练模型参数，其中，学习率Learning\_rate设置为0.001，Dropout为0.5，训练批次Epochs为50，批处理大小Batch\_size为32，
\begin{table}[htp] 
  \centering  
  \caption{模型参数设置}   
  \label{tab:parameter}  
