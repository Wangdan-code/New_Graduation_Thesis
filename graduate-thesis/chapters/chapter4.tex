...
为了更直观地展示本文提出的子树划分算法的有效性，本实验同样使用t-SNE技术将高维Token维度属性特征向量可视化。按照预先设定的参数，树表征学习后生成的张量维度是128，采用t-SNE技术将128维的张量转换为2维，得到的样本表征结果如图\ref{fig:astresult}所示。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/token}
  \caption{经过子树划分学习到的AST表征对比}\label{fig:astresult}
\end{figure}

分析上图，观察可视化结果，寻找可能的代码克隆群集。相似的代码片段在降维后的空间中应该更接近彼此。黑色代表label标签为1的样本，红色表示label标签为2的样本。经过树表征模型的训练，得到右图。
